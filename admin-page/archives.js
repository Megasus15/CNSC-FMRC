...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const resolveApiBaseUrl = () => {
    ...
    if (hostname === "localhost" || hostname === "127.0.0.1") return `${proto}//${hostname}:8000/api`;
  ...
  const statusPill = (text) => {
    ...
    let cls = "status-gray";
    if (["good", "active", "completed", "confirmed", "paid"].includes(s)) cls = "status-green";
    else if (["low", "pending", "incoming"].includes(s)) cls = "status-yellow";
    else if (["critical", "rejected", "cancelled", "archived"].includes(s)) cls = "status-red";
  ...
  const filterRecords = (records, fields) => {
    ...
    return records.filter((r) =>
      fields.some((f) => String(r[f] || "").toLowerCase().includes(searchQuery))
    );
  };
  ...
  const updatePagination = (meta, curPageEl, prevBtn, nextBtn, arr, page) => {
    ...
    if (meta)      meta.textContent = `Page ${page} of ${tp} • Showing ${from}–${to} of ${arr.length}`;
  ...
  const renderInventory = () => {
    const filtered = filterRecords(state.inventory.all, ["item_name", "description", "category", "unit", "status", "remarks"]);
    const paged    = paginate(filtered, state.inventory.page);
    updatePagination(invMeta, invCurPage, invPrev, invNext, filtered, state.inventory.page);
    ...
    invTbody.innerHTML = paged.map((item, idx) => {
      const rowNum = (state.inventory.page - 1) * PAGE_SIZE + idx + 1;
      const onHand = item.on_hand ?? 0;
      const statusText = onHand <= 0 ? "Critical" : onHand <= 5 ? "Low" : "Good";
      ...
      const remarksHtml = item.remarks ? `<span class="remarks-pill bg-gray-100 text-gray-500">${esc(item.remarks)}</span>` : `<span style="color:#9ca3af;font-size:0.75rem;">—</span>`;
      const baseDescriptionHtml = hasVariants ? "" : esc(item.description || "—");
      ...
      if (hasVariants) {
        variants.forEach((variant) => {
          ...
          const vStatus = variant.status || (vOnHand <= 0 ? "Critical" : vOnHand <= 5 ? "Low" : "Good");
  ...
  const renderAppointments = () => {
    const filtered = filterRecords(state.appointment.all, [
      "reference_no", "client_name", "contact_number", "email",
      "client_type", "purpose", "status",
    ]);
    const paged = paginate(filtered, state.appointment.page);
    updatePagination(apptMeta, apptCurPage, apptPrev, apptNext, filtered, state.appointment.page);
    ...
    apptTbody.innerHTML = paged.map((a) => `<tr>
  ...
  const renderOrders = () => {
    const filtered = filterRecords(state.order.all, [
      "order_no", "order_item", "customer_name", "payment_method", "lifecycle_status",
    ]);
    const paged = paginate(filtered, state.order.page);
    updatePagination(orderMeta, orderCurPage, orderPrev, orderNext, filtered, state.order.page);
    ...
    orderTbody.innerHTML = paged.map((o) => `<tr>
  ...
  const setError = (msg) => {
    const html = (cols) => `<tr><td colspan="${cols}"><div class="table-empty-state"><i class="fa-solid fa-triangle-exclamation" style="color:#ef4444;"></i><span style="color:#ef4444;">${msg}</span></div></td></tr>`;
  ...
  const loadArchives = async () => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        window.showAdminPopup?.("Your session has expired. Please login again.", {
      ...
      state.inventory.all   = Array.isArray(payload?.inventory)    ? payload.inventory    : [];
      state.appointment.all = Array.isArray(payload?.appointments) ? payload.appointments : [];
  ...
  invPrev?.addEventListener("click", () => {
    if (state.inventory.page > 1) { state.inventory.page--; renderInventory(); }
  });
  invNext?.addEventListener("click", () => {
    const filtered = filterRecords(state.inventory.all, ["item_name", "description", "category", "unit", "status", "remarks"]);
    if (state.inventory.page < totalPages(filtered)) { state.inventory.page++; renderInventory(); }
  });

  apptPrev?.addEventListener("click", () => {
    if (state.appointment.page > 1) { state.appointment.page--; renderAppointments(); }
  });
  apptNext?.addEventListener("click", () => {
    const filtered = filterRecords(state.appointment.all, ["reference_no", "client_name", "contact_number", "email", "client_type", "purpose", "status"]);
    if (state.appointment.page < totalPages(filtered)) { state.appointment.page++; renderAppointments(); }
  });

  orderPrev?.addEventListener("click", () => {
    if (state.order.page > 1) { state.order.page--; renderOrders(); }
  });
  orderNext?.addEventListener("click", () => {
    const filtered = filterRecords(state.order.all, ["order_no", "order_item", "customer_name", "payment_method", "lifecycle_status"]);
    if (state.order.page < totalPages(filtered)) { state.order.page++; renderOrders(); }
  });
  ...
  document.body.addEventListener("click", (e) => {
    ...
    if (toggleBtn) {
      ...
      const childRows = document.querySelectorAll(`.inv-variant-row[data-parent-inv="${parentId}"]`);
  ...
  btnConfirmRestore?.addEventListener("click", async () => {
    ...
    else if (module === "appointments") endpoint = `/appointments/${id}/unarchive`;
    ...
    try {
      ...
      window.showAdminPopup?.("Record restored successfully.", { title: "Success" });
