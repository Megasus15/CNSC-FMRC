...
async function loadServices() {
  ...
  try {
    ...
    grid.innerHTML = '<p class="empty-state">Failed to load services. Check backend connection.</p>';
  }
}
...
function renderCards() {
  ...
  if (!servicesData.length) {
    grid.innerHTML = '<div class="empty-state"><i class="fa-regular fa-folder-open" style="font-size:2rem;display:block;margin-bottom:10px;"></i>No services yet. Click "Add Service" to get started.</div>';
    return;
  }
...
function bindEvents() {
  ...
  document.getElementById('btnCancelService').addEventListener('click', closeModal);
  document.getElementById('btnCloseServiceModal').addEventListener('click', closeModal);
...
function openEdit(id) {
  ...
  document.getElementById('svcMaterials').value = (s.modal_materials || []).join('\n');
...
async function doSave(id) {
  const payload = {
    ...
    modal_materials:   document.getElementById('svcMaterials').value.split('\n').map(l => l.trim()).filter(Boolean),
    modal_best_for:    document.getElementById('svcBestFor').value.split('\n').map(l => l.trim()).filter(Boolean),
...
function doDelete(id, name) {
  window.showAdminConfirmPopup(`Delete "${name}"? It will be removed from both Services and Home pages.`, {
...
function setupChipInput(areaId, inputId) {
  ...
  input.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      ...
      if (val) { addChip(areaId, inputId, val); input.value = ''; }
    }
  });
}
...
function getChips(areaId) {
  return Array.from(document.getElementById(areaId).querySelectorAll('.chip-tag-item')).map(c => c.dataset.value);
}
