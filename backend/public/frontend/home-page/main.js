...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const showCustomerPopup = (message, options = {}) =>
        new Promise((resolve) => {
            ...
            const backdrop = popup.querySelector(
                ".admin-system-popup__backdrop",
            );

            if (titleEl)
                titleEl.textContent = options.title || "System Message";
            ...
            if (okBtn) {
                okBtn.textContent =
                    options.okText || (isConfirm ? "Confirm" : "Okay");
                okBtn.onclick = (ev) => {
                    ev?.stopPropagation();
                    closePopup(true);
                };
        ...
        if (openBtn) {
            if (modal) {
                ...
                if (card) {
                    ...
                    if (featureChips) {
                        ...
                        if (features.length) {
                            featureChips.innerHTML = features
                                .map(
                                    (f) =>
                                        `<span class="chip">${escHtmlModal(f)}</span>`,
                                )
                                .join("");
                            featureChips.style.display = "";
                            if (
                                subEl &&
                                subEl.classList.contains("modal-subtitle")
                            )
                                subEl.style.display = "";
                        } else {
                    ...
                    try {
                        bestFor = JSON.parse(
                            openBtn.dataset.bestFor ||
                                openBtn.dataset["best-for"] ||
                                "[]",
                        );
                    } catch {}
                    if (modalList2) {
    ...
    const isProductsPage =
        document.body.classList.contains("products-page-body");
    ...
    if (isProductsPage && productCards.length > 0) {
        ...
        const productInfoSummary =
            document.getElementById("productInfoSummary");
        ...
        `;
                ...
                starsContainer.addEventListener("click", (event) => {
                    const clickedStar =
                        event.target.closest(".rating-star-btn");
                    if (!clickedStar) return;

                    const selectedRating = parseInt(
                        clickedStar.dataset.star || "4",
                    );
                    card.dataset.userRating = String(selectedRating);
                    const scoreValue = ratingRow.querySelector(
                        ".rating-score-value",
                    );
                    if (scoreValue)
                        scoreValue.innerText = `${selectedRating}.0`;
            ...
            if (
                actionContainer &&
                !actionContainer.querySelector(".btn-view-info")
            ) {
        ...
        const applyProductFilters = () => {
            ...
            const cards = Array.from(
                shopGrid?.querySelectorAll(".shop-card") || [],
            );
            ...
            cards.forEach((card) => {
                ...
                const isOutOfStock =
                    stockBadge?.classList.contains("out-of-stock");
                const userRating = parseInt(card.dataset.userRating || "4");

                let visible = true;

                if (searchValue && !nameText.includes(searchValue))
                    visible = false;
                if (selectedCategory !== "all" && category !== selectedCategory)
                    visible = false;

                if (selectedFilter === "in-stock" && isOutOfStock)
                    visible = false;
                if (selectedFilter === "out-of-stock" && !isOutOfStock)
                    visible = false;
                if (selectedFilter === "top-rated" && userRating < 4)
                    visible = false;
            ...
            if (
                ...
                visibleCards.sort((a, b) => {
                    ...
                    const getPrice = (cardEl) => {
                        const priceText =
                            cardEl.querySelector(".product-price")?.innerText ||
                            "₱0";
    ...
    buyNowBtns.forEach((btn) => {
        btn.addEventListener("click", function (e) {
            ...
            if (card) {
                const imgScr = card.querySelector(
    ...
    const cartShortAddressText = document.getElementById(
        "cartShortAddressText",
    );
    ...
    const displayClientAddress = document.getElementById(
    ...
    const normalizeAddressEntry = (entry = {}) => ({
        ...
        phone_number: normalizePhoneDigits(
            entry.phone_number || entry.phone || "",
        ),
        ...
        customer_type:
            String(entry.customer_type || "Student").trim() || "Student",
    ...
    const clearCheckoutFormErrors = (mode) => {
        ...
        Object.values(ids).forEach((fieldId) =>
    ...
    const renderCheckoutAddress = () => {
        ...
        const phone =
            ...
            customerCheckoutProfile?.phone_number ||
            "";
        const addressLine =
        ...
        const department =
            selected?.department ||
            customerCheckoutProfile?.department ||
            "Not set";
        const role =
    ...
    const validateAddressForm = (mode) => {
        ...
        if (!department) {
            registerError(
                ids.dept,
                "Please enter your department or organization.",
            );
        }
    ...
    if (addressList) {
        addressList.addEventListener("click", (event) => {
            ...
            if (editBtn) {
                const editId = String(
                    editBtn.getAttribute("data-address-edit") || "",
                );
            ...
            const addressId = String(
                item.getAttribute("data-address-id") || "",
            );
            if (!addressId) return;
            ...
            try {
                ...
                if (useCartCheckout) {
                    const items = currentCheckoutItems.map((item) => {
                        ...
                        const lineUnitPrice = Number.isFinite(
                        ...
                        return {
                            ...
                            product_name: String(
                                item?.product_name || "Custom Order",
                            ),
                    ...
                    payload = {
                        ...
                        product_name:
                            checkoutTitle?.innerText?.trim() || "Custom Order",
                        product_image:
                            checkoutImg?.src || "/images/FMRC Logo.png",
                        quantity,
    ...
    const createCartItemCard = ({
        ...
        const qty = Math.max(
            1,
            Number.parseInt(String(quantity || "1"), 10) || 1,
        );
    ...
    const collectCartItemsFromDom = () => {
        ...
        return Array.from(
            ...
            const quantity = Math.max(
                1,
                Number.parseInt(
                    item.querySelector(".c-qty-input")?.value || "1",
                    10,
                ) || 1,
            );
            const checked = Boolean(
                item.querySelector(".cart-item-check")?.checked,
            );
            ...
            _cartSyncTimer = setTimeout(async () => {
                ...
                try {
                    ...
                    await fetchWithTimeout(
                        `${API_BASE_URL}/customer/cart/sync`,
                        {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
        ...
        if (customerSession.isAuthenticated) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    const apiItems = Array.isArray(payload?.data)
                        ? payload.data
                        : [];
                    ...
                    if (apiItems.length > 0) {
                        ...
                        if (
                            localStorage.getItem(storageKey) !== newCartString
                        ) {
            ...
            if (existingItem) {
                ...
                if (qtyInput) {
                    qtyInput.value = String(
                        Math.max(
                            1,
                            Number.parseInt(qtyInput.value || "1", 10) + 1,
                        ),
                    );
                }
    ...
    if (selectAllCartBtn) {
        selectAllCartBtn.addEventListener("change", (e) => {
            const checks =
                cartItemsContainer.querySelectorAll(".cart-item-check");
        ...
        cartCheckoutSubmitBtn.addEventListener("click", async () => {
            ...
            const selectedItems = checkedInputs
                .map((inputEl) => {
                    ...
                    const title =
                        card.querySelector("h4")?.innerText || "Custom Order";
                    const image =
            ...
            if (!selectedItems.length) {
                void showCustomerPopup(
                    "Please select at least one valid cart item.",
                    {
                        title: "Validation",
                    },
                );
            ...
            if (productIds.length > 0) {
                try {
                    ...
                    const stockPayload = await stockRes
                        .json()
                        .catch(() => ({}));
            ...
            if (checkoutImg)
                checkoutImg.src =
                    firstItem?.product_image || "/images/FMRC Logo.png";
            if (checkoutTitle) {
            ...
            if (
                ...
                currentMaxStock = 9999;
                setCheckoutStockNotice(
                    `Multiple products (${validatedItems.length})`,
                );
            }
    ...
    const getAppointmentAddress = () => {
        const country =
            document.getElementById("aptCountry")?.value?.trim() ||
            "Philippines";
    ...
    const fetchCalendarAvailability = async () => {
        try {
            ...
            (Array.isArray(data?.day_settings)
                ? data.day_settings
                : []
            ).forEach((entry) => {
    ...
    const clearAllFieldErrors = () => {
        document
            .querySelectorAll(".apt-input-group.has-error")
            .forEach((group) => {
                group.classList.remove("has-error");
    ...
    const validateAppointmentStep2 = () => {
        ...
        if (!mobile) {
            ...
            markError(
                "aptPhone",
                "Mobile Number is invalid. Use exactly 11 digits.",
            );
        }
    ...
    const populateReviewData = (step) => {
        ...
        const fName =
            document.getElementById("aptFName")?.value?.trim() || "N/A";
        const lName =
            document.getElementById("aptLName")?.value?.trim() || "N/A";
        ...
        const phone =
            document.getElementById("aptPhone")?.value?.trim() || "N/A";
        const email =
            document.getElementById("aptEmail")?.value?.trim() || "N/A";
        const purpose =
        ...
        const country =
            ...
            "Philippines";
        const notes =
            document.getElementById("aptDesc")?.value?.trim() || "N/A";
    ...
    const submitAppointment = async () => {
        ...
        if (!date || !time) {
            const msg =
                "Please select a date and time first before continuing.";
            showSlotMessage(msg);
    ...
    const downloadQrCodeCard = () => {
        ...
        img.onload = () => {
            ...
            ctx.fillText(
                "Scan to open the official verification page",
                180,
                988,
            );
    ...
    const getRenderableSlotsForDate = (dateKey) => {
        ...
        const seen = new Set(
            baseSlots.map((slot) => `${slot.label}|${slot.type}`),
        );
    ...
    const renderCalendar = (month, year) => {
        ...
        for (let day = 1; day <= daysInMonth; day += 1) {
            ...
            if (
                Array.isArray(daySettings.events) &&
                daySettings.events.length
            ) {
                cell.setAttribute(
                    "title",
                    `Event: ${daySettings.events.join(", ")}`,
                );
            }
    ...
    aptLName?.addEventListener("input", () => {
        aptLName.value = aptLName.value
            .replace(/[^A-Za-z\s]/g, "")
            .slice(0, 20);
    });

    aptFName?.addEventListener("input", () => {
        aptFName.value = aptFName.value
            .replace(/[^A-Za-z\s]/g, "")
            .slice(0, 25);
    });
    ...
    if (appointmentBtn && appointmentOverlay) {
        appointmentBtn.addEventListener("click", () => {
            ...
            void (async () => {
                await fetchCalendarAvailability();
                if (!appointmentOverlay.classList.contains("show-modal"))
                    return;
    ...
    bindClick("btnGoToConfirm", () => {
        ...
        if (!date || !time) {
            showSlotMessage(
                "Please select a date and time first before proceeding.",
            );
            return;
        }
        clearSlotMessage();
    ...
    if (contactMessageForm) {
        contactMessageForm.addEventListener("submit", async (event) => {
            ...
            const submitBtn = contactMessageForm.querySelector(
                ".contact-submit-btn",
            );
...
(() => {
    ...
    const ensureStatusModal = () => {
        ...
        if (!modal) {
            ...
            modal.innerHTML =
                '<div class="status-box" id="userStatusText"></div>';
            document.body.appendChild(modal);
        }
        return {
            modal,
      ...
      `;
        ...
        if (form) {
            form.onsubmit = async (event) => {
                ...
                const newPassword =
                    overlay.querySelector("#cp_new")?.value || "";
    ...
    const openOrdersModal = (activeUserInfo) => {
      ...
      `;
            ...
            const tabs = Array.from(
                overlay.querySelectorAll(".customer-orders-tab"),
            );
            ...
            const detailModal = overlay.querySelector(
                "#customerOrderDetailModal",
            );
            const detailContent = overlay.querySelector(
                "#customerOrderDetailContent",
            );
            const detailTitle = overlay.querySelector(
                "#customerOrderDetailTitle",
            );
            const closeDetailBtn = overlay.querySelector(
            ...
            const startRealtimeRefresh = () => {
                ...
                state.refreshTimer = window.setInterval(() => {
                    if (!overlay.classList.contains("show") || document.hidden)
                        return;
      ...
      `;
            ...
            const renderOrders = () => {
                if (state.loading) {
                    ...
                    tabs.forEach((tab) => {
                        const countEl = tab.querySelector(
            ...
            const renderDetailModal = (detail) => {
                ...
                const safeOrderNo = escapeHtml(
                    detail.order_no_display ||
                        `#${detail.order_no || detail.id || "-"}`,
                );
                const safeTitle = escapeHtml(
                    detail.product_name || "Order Details",
                );
                ...
                const courierName = escapeHtml(
                    detail.courier_name || "J&T Express",
                );
        ...
        `;
          ...
          `;
            ...
            const refreshActiveDetail = async (force = false) => {
                ...
                if (!force && Date.now() - state.lastDetailRefreshAt < 1000)
                    return;
                ...
                try {
                    ...
                    if (detail) {
                        state.detailsById.set(
                            String(state.activeDetailId),
                            detail,
                        );
                        renderDetailModal(detail);
                        state.lastDetailRefreshAt = Date.now();
            ...
            const refreshOrders = async (showLoading, force = false) => {
                ...
                if (
                    ...
                    now - state.lastRefreshAt <
                        CUSTOMER_ORDERS_MIN_REFRESH_GAP_MS
                ) {
                    return;
                }
            ...
            document.addEventListener("visibilitychange", () => {
                if (document.hidden || !overlay.classList.contains("show"))
                    return;
                const popup = document.getElementById("customerSystemPopup");
                if (popup && popup.classList.contains("show")) return;
                state.lastDetailRefreshAt = 0;
            ...
            customerOrdersController = {
                open: async (nextUserInfo) => {
                    ...
                    const cachedOrders = customerOrdersCache.get(
                        state.cacheKey,
                    );
    ...
    function initCarousel(track) {
        ...
        function upd() {
            items.forEach(function (it, i) {
                ...
                else if (i === (cur + 1) % items.length)
                    it.classList.add("next");
