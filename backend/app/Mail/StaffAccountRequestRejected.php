...
class StaffAccountRequestRejected extends Mailable
{
     ...
     * Build the HTML body.
     *
     * The header, body copy and header colour come from the admin-editable
     * `staff_request_rejected` template. The request summary, the quoted
     * administrator note, the "what to do next" card, the Reach Us Anytime
     * table and the closing customer-account line are code-owned — the closing
     * line carries links, so it stays in code rather than in a text field.
     *
    ...
    private function buildHtml(): string
    {
        ...
        $accent = EmailTemplate::color(EmailTemplate::resolve('staff_request_rejected')['header_color']);
        $declined = '#b42318';
...
HTML;
        ...
        return EmailTemplate::render('staff_request_rejected', [
            'staff_name' => $row->displayName(),
